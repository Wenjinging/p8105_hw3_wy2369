...
Clean data first, and create separate variables for **year**, **month**, and **day**. Using the code chunk below to convert variables **prcp**, **snow**, **snwd**, **tmax** and **tmin** into numeric variables. Also convert the unit of tmax and tmin to Degree C. 
...
Now there are **`r nrow(ny_noaa_df)`** rows and **`r ncol(ny_noaa_df)`** columns in the `ny_noaa_df` dataset. Variables are **`r ncol(ny_noaa_df)`** which include id, year, month, day, prcp(mm), snow(mm), snwd(mm), tmax(degree C), and tmin(degree C).   

For snowfall, The most commonly observed values is

### Make a two-panel plot showing the average max temperature in January and July

 in each station across years. Is there any observable / interpretable structure? Any outliers?
